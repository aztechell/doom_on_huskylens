...
#ifndef HK_LCD_ENABLE_SHADOW
#define HK_LCD_ENABLE_SHADOW 1
#endif

static uint8_t g_line[LCD_W * 2];
#if HK_LCD_ENABLE_SHADOW
static uint16_t g_lcd_shadow[LCD_W * LCD_H] __attribute__((aligned(4), section(".bss")));
...
uint16_t lcd_shadow_pixel(uint16_t x, uint16_t y)
{
    ...
    if(x >= LCD_W || y >= LCD_H)
        return COLOR_BLACK;
    return g_lcd_shadow[(uint32_t)y * LCD_W + x];
#else
    (void)x;
    (void)y;
    return COLOR_BLACK;
...
void lcd_shadow_write_pixels(const uint8_t *data, size_t len)
{
    ...
    {
        uint16_t color = (uint16_t)((uint16_t)data[i * 2U] << 8) | data[i * 2U + 1U];
        if(g_lcd_cursor_x < LCD_W && g_lcd_cursor_y < LCD_H)
            g_lcd_shadow[(uint32_t)g_lcd_cursor_y * LCD_W + g_lcd_cursor_x] = color;
